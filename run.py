...
def export_to_google_sheets(events):
    log.info(f"Tentative Google Sheets avec ID : {GOOGLE_SHEET_ID[:20]}...")
    if not GOOGLE_SHEET_ID:
        log.info("Google Sheets désactivé (GOOGLE_SHEET_ID vide).")
        return
    try:
        import gspread
    ...
    try:
        ...
        log.error(f"credentials.json introuvable")
...
if __name__ == "__main__":
    args = sys.argv[1:]
    if "--schedule" in args:       run_scheduled()
    elif "--seed-only" in args:    run("seed-only")
